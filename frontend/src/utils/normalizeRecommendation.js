...
function formatTransport(transport) {
  ...
  if (transport.mode === 'car') return '자동차'
...
export function normalizeRecommendation(response) {
  return {
    message: (response.recommendation_message ?? '지금 가기 좋은 곳을 찾았어요.')
      .split('\n\n')[0]
      .replaceAll('**', ''),
    mapContext: response.recommendation_context ? {
      ...response.map_context,
      start: response.recommendation_context.start_location,
      end: response.recommendation_context.end_location ?? null,
