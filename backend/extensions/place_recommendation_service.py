...
def normalize_kakao_places(
    ...
    for place in places:
        ...
        normalized_places.append({
            ...
            # Kakao Local은 대표 사진을 제공하지 않지만 상세 페이지 URL은
            # 제공한다. 사진 제공처를 붙일 때 장소 식별·검증에 사용할 수
            # 있도록 응답 계약에 보존한다.
            "place_url": place.get("place_url"),
            "image_url": place.get("image_url"),
            "image_source": place.get("image_source"),
            "image_attribution": place.get("image_attribution"),
...
def normalize_tour_places(
    ...
    for place in places:
        ...
        normalized_places.append({
            ...
            # Tour/제휴 응답이 대표 이미지를 제공하는 경우 프론트 카드가
            # 별도 변경 없이 실제 사진을 사용할 수 있게 공통 필드로 전달한다.
            "image_url": (
                place.get("image_url")
                or place.get("imageUrl")
                or place.get("firstimage")
                or place.get("firstImage")
            ),
            "image_source": place.get("image_source") or "tour",
            "image_attribution": place.get("image_attribution"),
...
def recommend_places(
    ...
    active_activities.sort(
        key=lambda activity: -int(preferences.get(activity, 0))
    )
    ...
    if "culture" in active_activities:
        ...
        except SeoulCultureAPIError as error:
            logger.warning("[seoul-culture-error] %s", error)
            normalized_seoul_culture_places = []
