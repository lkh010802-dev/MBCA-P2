...
DEFAULT_MAX_CLASSIFICATION_REVIEW_COMMIT = 2
DEFAULT_MAX_DUPLICATE_REVIEW_COMMIT = 10
...
def get_master_commit_block_reasons(
    ...
    *,
    ...
    """Return only review conditions large enough to block the daily commit.

    One or two ambiguous classification rows are already excluded from canonical.
    Ambiguous duplicate edges are not auto-merged, so both records stay separate.
    A small batch of these cases is therefore a warning, not a reason to discard the
    whole daily output. The default duplicate-review allowance is 10; larger spikes
    still block the commit so source/parser regressions remain visible.
    """
    reasons: list[str] = []
    if len(classification_review) > max_classification_review_commit:
        reasons.append(
            f"classification_review={len(classification_review)} "
            f"> allowed={max_classification_review_commit}"
        )
    if len(duplicate_review) > max_duplicate_review_commit:
        reasons.append(
            f"duplicate_review={len(duplicate_review)} "
...
def main() -> None:
    ...
    review_protected_source_refs = {
        (str(row.get("source") or ""), str(row.get("source_id") or ""))
        for row in classification_review
    ...
    tolerated_duplicate_review_count = (
        ...
        if 0 < len(duplicate_review) <= args.max_duplicate_review_commit
        else 0
    )
    ...
    if tolerated_duplicate_review_count:
        print(
            f"중복 검토 {tolerated_duplicate_review_count}건: 자동 병합하지 않고 각각 유지한 채 전체 반영 계속"
        )
