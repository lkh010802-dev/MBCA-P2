...
def clean_description(value: object) -> str | None:
    """Clean human-readable detail text and drop leaked API payload fragments.

    Some DayForYou/Lotte detail pages occasionally expose a truncated JSON/API
    response in the content area, e.g. ``{"seq":"...","totCnt":0,...}``.
    Such fragments are not useful descriptions and should not reach canonical
    CSV/JSON output. The check is deliberately narrow: at least two known API
    field markers must be present.
    """
    text = clean_text(value)
    if not text:
        return None
    marker_count = sum(marker in text for marker in _BROKEN_DESCRIPTION_MARKERS)
    if marker_count >= 2:
        return None
    return text
...
def common_from_dayforyou(
    ...
    *,
    ...
    item.update({
        "description": clean_description(row.get("detail_summary")),
