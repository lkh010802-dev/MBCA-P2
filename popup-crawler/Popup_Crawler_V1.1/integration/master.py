...
def update_master(
    ...
    *,
    ...
    protected_source_refs: set[tuple[str, str]] | None = None,
    ...
    for old_id, old in old_by_id.items():
        ...
        if old_refs and old_refs.issubset(retired_source_refs):
            ...
            continue
        # A source row held for classification review is intentionally omitted
        # from today's canonical. Preserve the matching old master row as-is
        # instead of treating that omission as a real disappearance. It remains
        # out of today's backend CSV because seen_in_latest_run is false.
        if old_refs & protected_source_refs:
            protected = dict(old)
            protected["seen_in_latest_run"] = False
            protected["master_updated_at"] = run_timestamp
            protected["skipped_due_to_review"] = True
            result.append(protected)
            review_protected_count += 1
            continue
        absent = dict(old)
    ...
    report = {
        ...
        "review_protected_master_count": review_protected_count,
