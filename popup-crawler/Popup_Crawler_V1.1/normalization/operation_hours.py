...
def _clean(value: Any) -> str:
    ...
    # A Popga detail field has been observed as ``메일 10:30 ~ 22:00``.
    # In an operation-hours-only field this is a low-risk source typo for
    # ``매일``. Restrict the repair to a standalone token directly before a
    # clock so ordinary text containing the word '메일' is never changed.
    text = re.sub(
        r"(?<![가-힣A-Za-z0-9])메일(?=\s*(?:[01]?\d|2[0-3]):[0-5]\d)",
        "매일",
        text,
    )
    return text
...
def _extract_days(
    ...
    *,
    ...
    # ``요일`` is unambiguous and may appear anywhere in the schedule label.
    for token in re.findall(r"([월화수목금토일])요일", scrubbed):
        found.append(KOR_DAY[token])

    # A bare Korean weekday is only one syllable and is ambiguous in prose:
    # ``할 수 있습니다`` must never mean Wednesday. Bare day tokens therefore
    # count only in the schedule prefix before the first clock/closed marker.
    # Ranges such as 월-금 are already handled above.
    first_clock = re.search(rf"(?<!\d){TIME_TOKEN}(?!\d)", scrubbed)
    first_closed = CLOSED_RE.search(scrubbed)
    cut_points = [
        match.start() for match in (first_clock, first_closed) if match is not None
    ]
    day_context = scrubbed[:min(cut_points)] if cut_points else ""
    bare_day_re = re.compile(
        r"(?<![가-힣A-Za-z0-9])([월화수목금토일])(?![가-힣A-Za-z0-9])"
    )
    for match in bare_day_re.finditer(day_context):
        found.append(KOR_DAY[match.group(1)])
...
def resolve_schedule_for_day(
    ...
    if active and not closed:
        ...
        today_closed = True
    elif not matching and schedule:
        # If the source published an explicit weekday schedule (for example
        # ``수-토 13:00~19:00``), an omitted ordinary weekday means the place
        # is not open that day. This is different from having no schedule at
        # all, which remains unknown.
        today_closed = True
    else:
        # No published schedule at all, or conflicting active/closed records.
        today_closed = None
