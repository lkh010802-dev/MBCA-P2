...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--fresh",
        action="store_true",
        help="오늘 목록을 새로 요청(상세 HTML은 TTL 캐시를 계속 사용할 수 있음)",
    )
    parser.add_argument(
        "--no-detail-cache",
        action="store_true",
        help="DayForYou 상세 HTML 캐시를 무시하고 전부 live fetch",
    )
    parser.add_argument(
        "--execute",
        action="store_true",
...
def main() -> None:
    ...
    configured_cache = str(os.getenv("DAYFORYOU_DETAIL_CACHE_DIR") or "data/cache/detail_html")
    cache_dir = run_dir / "detail_html" if args.no_detail_cache else Path(configured_cache)
    cache_hours = 0.0 if args.no_detail_cache else float(os.getenv("DAYFORYOU_DETAIL_CACHE_HOURS", "54"))
    # Operating hours live on DayForYou detail pages, so every Seoul card is
    # parsed. The HTML itself is cached with a TTL; new/expired IDs are fetched
    # live while unchanged recent cards are reparsed locally.
    detail_records = fetch_details(
        normalized,
        cache_dir=cache_dir,
        cache_max_age_hours=max(0.0, cache_hours),
    )
    ...
    report = {
        ...
        "fresh_list_fetch": args.fresh,
        "detail_cache_dir": str(cache_dir),
        "detail_cache_hours": cache_hours,
        "detail_cache_hits": sum(bool(x.get("fetched_from_cache")) for x in details),
        "detail_live_fetches": sum(not bool(x.get("fetched_from_cache")) for x in details),
