...
def validate_dayforyou(
    ...
    *,
    ...
    metrics = {
        ...
        "detail_cache_hits": int(report.get("detail_cache_hits") or 0),
        "detail_live_fetches": int(report.get("detail_live_fetches") or 0),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Popply 상세 수집 허용 실패율. 격리 가능한 일시적 hydration 실패를 고려해 기본 0.06",
    )
    parser.add_argument(
        "--max-popply-core-incomplete-count",
        type=int,
        default=int(os.getenv("DAILY_MAX_POPPLY_CORE_INCOMPLETE_COUNT", "10")),
        help="Popply 상세 핵심필드 불완전 quarantine 허용 건수. 기본 10",
    )
    parser.add_argument(
        "--max-popply-core-incomplete-rate",
        type=float,
        default=float(os.getenv("DAILY_MAX_POPPLY_CORE_INCOMPLETE_RATE", "0.06")),
        help="Popply 상세 핵심필드 불완전 quarantine 허용 비율. 기본 0.06",
    )
    parser.add_argument(
        "--sequential-sources",
        action="store_true",
...
def _summary_text(report: dict[str, Any]) -> str:
    ...
    if integration:
        lines.extend([
            ...
            f"duplicate_quarantine={integration.get('duplicate_quarantine_record_count', 0)}",
...
def main() -> None:
    ...
    if not (0.0 <= args.max_popply_core_incomplete_rate <= 1.0):
        raise SystemExit("--max-popply-core-incomplete-rate must be between 0 and 1")
    if args.max_popply_core_incomplete_count < 0:
        raise SystemExit("--max-popply-core-incomplete-count must be >= 0")
    ...
    try:
        if str(os.getenv("POPUP_PRODUCTION") or "").strip().lower() in {"1", "true", "yes", "on"}:
            state_script = ROOT / "scripts" / "ensure_production_state.py"
            state_started = time.monotonic()
            state_proc = subprocess.run(
                [python, str(state_script)],
                cwd=ROOT,
                text=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT,
            ...
            state_output = state_proc.stdout or ""
            print(state_output, end="" if state_output.endswith("\n") else "\n", flush=True)
            report["production_state"] = {
                "enabled": True,
                "master_path": str(os.getenv("POPUP_MASTER_PATH") or "data/master/canonical_master.jsonl"),
                "returncode": state_proc.returncode,
                "duration_seconds": round(time.monotonic() - state_started, 2),
                "output": state_output,
            }
            if state_proc.returncode != 0:
                report["status"] = "BLOCKED_STATE"
                report["errors"].append("production master state is missing or could not be recovered")
                raise RuntimeError("production state gate blocked crawler before source fetch")
        ...
        classification_review_count = int(integration_report.get("classification_review_count") or 0)
        duplicate_review_count = int(integration_report.get("duplicate_review_count") or 0)
        review_tolerance = integration_report.get("review_tolerance") or {}
        max_classification_review = int(review_tolerance.get("max_classification_review_commit") or 0)
        max_duplicate_review = int(review_tolerance.get("max_duplicate_review_commit") or 0)

        # run_integrate already blocks when review counts exceed the configured
        # tolerance. If it returned 0, the remaining 1-2 ambiguous records are
        # intentionally skipped/kept separate and should be warnings only.
        if classification_review_count:
            report["warnings"].append(
                f"integration: classification_review={classification_review_count}; "
                f"ambiguous record(s) excluded and commit continued "
                f"(allowed <= {max_classification_review})"
            )
        if duplicate_review_count:
            quarantine_count = int(integration_report.get("duplicate_quarantine_record_count") or 0)
            report["warnings"].append(
                f"integration: duplicate_review={duplicate_review_count}; "
                f"{quarantine_count} ambiguous record(s) quarantined from today's output; "
                f"safe rows committed (spike gate <= {max_duplicate_review} records)"
            )
