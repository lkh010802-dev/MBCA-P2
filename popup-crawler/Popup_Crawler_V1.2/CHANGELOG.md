...
- Popga 무한 스크롤의 다음 페이지 요청이 느릴 때 24건에서 조기 종료되지 않도록, fetch spinner 완료를 기다린 후 목록 안정 여부를 판단합니다.
- Popga 목록이 이전 실행의 65% 미만이면 더 긴 로딩 대기로 목록 단계만 한 번 자동 재시도하고, 두 시도 중 더 완전한 결과를 사용합니다.
- 실패·부분 실행이 다음 날 캐시와 품질 비교 기준을 덮어쓰지 않도록, 최근 7일의 정상 규모 전체 실행만 기준으로 선택합니다.

## 1.2 Popply performance hotfix - 2026-09-11

- 변경 없는 ACTIVE 상세 캐시의 최대 유효기간을 54시간에서 168시간으로 늘렸습니다.
- 48시간이 지난 캐시는 가장 오래된 항목부터 하루 최대 20건만 분산 갱신해 대량 동시 만료를 방지합니다.
- 새 행사, 목록 핵심값이 변경된 행사, UPCOMING 행사는 기존처럼 live 상세를 확인합니다.
- Popply live 상세를 두 worker로 제한 병렬화하고 결과는 원래 목록 순서대로 저장합니다.
- 현재 Next.js 서버 렌더링 HTML을 일반 HTTP로 먼저 파싱하고, 불완전한 예외 페이지만 Playwright로 재시도합니다.
- 상세 파싱에 불필요한 stylesheet를 차단하고 전체 DOM 대신 필요한 상세 DOM을 기준으로 기다립니다.
- live 상세별 소요 시간과 실행 최대 소요 시간, 캐시 계획을 report에 기록합니다.
