            ...
            try:
                for attempt in (1, 2):
                    ...
                    if attempt == 1:
                        ...
                        # Some Popply pages hydrate slowly. Reloading here resets
                        # hydration and can repeatedly prevent the detail fields
                        # from ever reaching the ready state. Keep waiting on the
                        # same document instead.
                        wait_ms = 22_000

                    try:
                        _wait_for_core_detail(
                            page,
                            timeout_ms=wait_ms,
                        )
                    except PlaywrightTimeoutError:
                        # Parse once more even after the wait because the DOM can
                        # finish updating very close to the timeout boundary.
                        pass
                    if settle_ms:
