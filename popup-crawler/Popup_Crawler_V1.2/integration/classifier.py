...
EXPLICIT_POPUP_RE = re.compile(r"팝업|POP\s*[-–—]?\s*UP", re.I)
...
BRAND_COMMERCE_RE = re.compile(
    r"상품|제품|굿즈|물건|판매|구매|쇼핑|랜덤\s*박스|스토어|마켓|쇼룸",
    re.I,
)
...
DAYFORYOU_RECRUITMENT_EVENT_RE = re.compile(
    r"\bCONTEST\b|지원자\s*모집|참가자\s*모집|오디션",
    re.I,
)
DAYFORYOU_TICKETING_RE = re.compile(
    r"얼리버드|티켓\s*(?:오픈|판매|예매)|예매\s*(?:오픈|시작)",
    re.I,
)
DAYFORYOU_CULTURAL_VENUE_RE = re.compile(
    r"^\s*\[[^\]]*(?:뮤지엄|미술관|박물관)[^\]]*\]",
    re.I,
)
DAYFORYOU_ROUNDUP_TITLE_RE = re.compile(
    r"(?:팝업\s*(?:리스트|목록|모음|정리|추천)|POP\s*[-–—]?\s*UP\s*LIST)",
    re.I,
)
...
def classify_dayforyou_final(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if DAYFORYOU_ROUNDUP_TITLE_RE.search(name):
        return _result(row, "NON_POPUP", "popup_roundup_or_list_post", 0.99)
    ...
    if DAYFORYOU_RECRUITMENT_EVENT_RE.search(name):
        return _result(row, "NON_POPUP", "recruitment_or_contest_title", 0.99)
    if DAYFORYOU_TICKETING_RE.search(name):
        return _result(row, "NON_POPUP", "ticketing_or_earlybird_title", 0.99)
    if DAYFORYOU_CULTURAL_VENUE_RE.search(name):
        return _result(row, "NON_POPUP", "cultural_venue_title_without_popup_signal", 0.97)
...
def classify_popply(row: dict[str, Any]) -> dict[str, Any]:
    ...
    raw_category = str(raw.get("category_raw") or "")
    raw_event_type = str(raw.get("event_type_raw") or "")

    category_evidence = {
        value.strip()
        for value in (category, raw_category, raw_event_type)
        if value.strip()
    }
    ...
    if category_evidence & CULTURAL_NON_POPUP_CATEGORIES or CULTURAL_EVENT_TITLE_RE.search(name):
    ...
    if (
        "브랜드/캠페인" in category_evidence
        and days is not None
        and days <= 30
        and BRAND_COMMERCE_RE.search(f"{combined} {tags}")
    ):
        return _result(row, "POPUP", "short_brand_commerce_event", 0.92)
