#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Run from a V1.3 FREEZE_WITH_BLIND100_V2 copy after replacing intent_postprocess.py.
Checks all 310 known expected rows remain fixed points of the patched postprocessor.
"""
import json
...
SETS=[
 ...
 ('blind100_v2', HERE/'tests/blind100_v2/golden.jsonl'),
]

def semantic_errors(e):
    errs=[]
    if e.get('start_time') is not None and e.get('start_time_period') is not None: errs.append('START_TIME_PERIOD_CONFLICT')
    if e.get('end_time') is not None and e.get('end_time_period') is not None: errs.append('END_TIME_PERIOD_CONFLICT')
    t,s=e.get('target_location_text'),e.get('target_location_scope')
    if (t is None)!=(s is None): errs.append('TARGET_SCOPE_LOCATION_CONFLICT')
    lo,hi=e.get('desired_duration_min_minutes'),e.get('desired_duration_max_minutes')
    if lo is not None and hi is not None and lo>hi: errs.append('DURATION_RANGE_INVALID')
    for f in ('activities','companions'):
        vals=e.get(f,[])
        if isinstance(vals,list) and len(vals)!=len(set(vals)): errs.append(f.upper()+'_DUPLICATE')
    return errs
