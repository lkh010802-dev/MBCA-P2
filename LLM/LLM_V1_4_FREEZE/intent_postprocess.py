...
V1.3.1 freeze: V1.3 FIX1 + conservative Blind100-v2 failure corrections.
"""
...
# Conservative compact Korean desired-duration ranges used by the policy.
_KOREAN_COMPACT_DURATION_RANGE_RE = re.compile(
    r"(?:지금부터\s*|앞으로\s*)?"
    r"(?P<range>한두|두세)\s*시간"
    r"\s*(?:정도\s*|동안\s*)?(?:그냥\s*)?"
    r"(?P<cue>뭐\s*하지|뭘\s*하지|뭐\s*할까|뭘\s*할까|놀고\s*싶어|시간\s*보내고\s*싶어)"
)
_KOREAN_COMPACT_DURATION_RANGE_MINMAX = {
    "한두": (60, 120),
    "두세": (120, 180),
}
...
def postprocess_intent(user_input: str, runtime_context: dict, predicted: dict):
    ...
    # Rule C2: compact Korean ranges such as "한두 시간 뭐할까" are desired duration.
    # Keep this separate from Rule C so a range is never misread as the trailing single number.
    range_match = _KOREAN_COMPACT_DURATION_RANGE_RE.search(user_input)
    if range_match and not _AVAILABILITY_RE.search(user_input):
        min_minutes, max_minutes = _KOREAN_COMPACT_DURATION_RANGE_MINMAX[range_match.group("range")]

        if out.get("end_time") is not None:
            changes.append({"field":"end_time","from":out.get("end_time"),"to":None,
                            "reason":"open_ended_duration_range_not_availability"})
            out["end_time"] = None
        if out.get("end_time_period") is not None:
            changes.append({"field":"end_time_period","from":out.get("end_time_period"),"to":None,
                            "reason":"open_ended_duration_range_not_availability"})
        ...
        if out.get("desired_duration_min_minutes") != min_minutes:
            changes.append({"field":"desired_duration_min_minutes",
                            "from":out.get("desired_duration_min_minutes"),"to":min_minutes,
                            "reason":"open_ended_desired_duration_range"})
            out["desired_duration_min_minutes"] = min_minutes
        if out.get("desired_duration_max_minutes") != max_minutes:
            changes.append({"field":"desired_duration_max_minutes",
                            "from":out.get("desired_duration_max_minutes"),"to":max_minutes,
                            "reason":"open_ended_desired_duration_range"})
            out["desired_duration_max_minutes"] = max_minutes
    ...
    # Rule F2: normalize an already-detected single start period to the literal Korean label.
    # This does not create a period from null, and it avoids next-schedule periods handled above.
    period_marks = list(_PERIOD_MARK_CAPTURE_RE.finditer(user_input))
    if (
        out.get("start_time") is None
        and out.get("start_time_period") is not None
        and len(period_marks) == 1
        and not _NEXT_SCHEDULE_CUE_RE.search(user_input[period_marks[0].end():])
    ):
        literal_enum = _PERIOD_WORD_TO_ENUM[period_marks[0].group("word")]
        if out.get("start_time_period") != literal_enum:
            changes.append({"field":"start_time_period","from":out.get("start_time_period"),
                            "to":literal_enum,"reason":"literal_period_label_normalization"})
            out["start_time_period"] = literal_enum

    # Rule G: culture requires an explicit culture cue under the current policy.
    # Generic "구경" or the name of a landmark alone does not imply culture.
    if "culture" in (out.get("activities") or []) and not _EXPLICIT_CULTURE_CUE_RE.search(user_input):
        new_activities = [x for x in out.get("activities", []) if x != "culture"]
        changes.append({"field":"activities","from":out.get("activities"),"to":new_activities,
                        "reason":"culture_without_explicit_culture_cue"})
        out["activities"] = new_activities
